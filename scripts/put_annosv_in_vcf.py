...
def parse_args(args):
    ...
    parser.add_argument("-o", "--output", default="/dev/stdout", help="File to write (stdout)")
    #parser.add_argument("-p", action="store_true", help="Pass only")
...
def make_anno_lookup(anno):
    ...
    def make_key(dat):
        ...
        """
        return "%s:%s.%s.%s" % (data[1], data[2], data[8], data[9])
    ...
    def extract_anno(data, header):
        ...
        for v,k in zip(data[-len(header):], header):
            if v == "": continue
            #if k == "Annotation_mode" and v == "split": continue
    ...
    for i in glob.glob(anno):
        with open(i) as fh:
            ...
            #annos = [_.replace(' ', '_') for _ in header[header.index('Annotation_mode'):]]
            annos = [_.replace(' ', '_') for _ in header[header.index('AnnotSV type'):]]
            for line in fh:
                data = line.strip().split('\t')
                manno = extract_anno(data, annos)
                if data[0] == "split": continue
                ret[make_key(data)] = manno
    return ret
...
def main(args):
    ...
    isZip = args.vcf.endswith(".gz")
    output = open(args.output, 'w')
    v = open(args.vcf) if not isZip  else gzip.GzipFile(args.vcf)
    cnt = 0
    pos = -1
    nannod = 0
    noanno = 0
    for entry in v:
        ...
        if entry.startswith("##"):
            ...
            #else:
            #    chrom_key = data[0]
            #if args.p and data[6] != 'PASS':
            #    continue
            cnt += 1
            k = "%s:%s.%s.%s" % (data[0][3:], data[1], data[3], data[4])
            if k in anno_lookup:
    ...
    sys.stderr.write("%d entries written. %d entries annotated, %d entries not annotated\n" % (cnt, nannod, noanno))
...
##INFO=<ID=AnnotSV_type,Number=1,Type=String,Description="AnnotSV type">
"""
